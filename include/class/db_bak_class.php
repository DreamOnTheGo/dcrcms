...
class DB_BAK{
	...
	/**
	 * 函数SetTables 设置本类要操作的表
 	 * @param array $talbe_list 要操作的表
	 * @return boolean(true)
	 */
	function SetTables($table_list){
		$this->table_list=$table_list;
	}
	/**
	 * 函数GetCreateTableSql 获取表的创建语句 版本号>=1.0.4
 	 * @param boolean $is_add_drop 是否添加drop table语句
	 * @return array 成功返回表的创建语句
	 */
	function GetCreateTableSql($is_add_drop=true){
		$t_str='';
		//
	...
	/**
	 * 函数Get_Table_Data 返回单个数据表的数据 主要是insert数据 版本号>=1.0.4
	 ...
	 */
	function Get_Table_Data($table_name,&$result_arr,/*$space=2097152*/$space=1000000){
		$c_index=0;
		$sql_bak="select * from $table_name";
		$table_cols=$this->db->GetTableCol($table_name);
