...
class CKFinder_Connector_CommandHandler_FileUpload extends CKFinder_Connector_CommandHandler_CommandHandlerBase
{
    ...
    public function sendResponse()
    {
		...
		$sFileName=date(ymdhms).rand(1000,9999).".".$sExtension;
		//dcr e
		...
		/*
		session_start();
		$web_dir=$_SESSION['web_dir'];
		$t_dir=split('\\',$sServerDir);
		if(!$t_dir)$t_dir=split('\/',$sServerDir);
		$t_dir_c=count($t_dir);
		$t_dir[$t_dir_c-1]=$t_dir[$t_dir_c-2];
		$t_dir[$t_dir_c-2]=substr($web_dir,1);
		//$t_dir[$t_dir_c-2]=str_replace('\\','',(str_replace('\/','',$web_dir));
		$sServerDir=implode(DIRECTORY_SEPARATOR,$t_dir);
		//print_r($t_dir);
		//echo $sServerDir;
		//exit;
		*/
		...
		$now=time();
		$yearDir=$sServerDir.'/'.date('Y_m',$now).'/';
		if (!file_exists($yearDir)&&!is_dir($yearDir)){
		mkdir($yearDir,0777);
		}
		$sServerDir=$yearDir;
		$oRegistry->set("FileUpload_url", $this->_currentFolder->getUrl().date('Y_m',$now).'/');//这一个要传递
