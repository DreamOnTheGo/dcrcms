...
<table cellSpacing=0 cellPadding=0 width="95%" align=center border=0>
  ...
  ?>
    <td style="padding:3px;"><a href="fmanage.php?cpath=<?php echo $pdir;?>">上级目录</a> 当前目录：<?php echo $cpath; ?></td></tr>
  </table>
...
<table cellSpacing=2 cellPadding=5 width="95%" align=center border=0 bgcolor="#ecf4fc">  
  ...
  <?php
	...
	for($i=0;$i<$dir_num;$i++){
		if(eregi("^_(.*)$",$dir_list[$i]['filename'])) continue;
		if(eregi("^\.(.*)$",$dir_list[$i]['filename'])) continue;
	?>    
  ...
  ?>  
  <tr bgcolor="#FFFFFF" onMouseMove="javascript:this.style.backgroundColor='#F4F9EB';" onMouseOut="javascript:this.style.backgroundColor='#FFFFFF';">
    <td><img border="0" src="images/file.gif" /><a href="/<?php echo str_replace(WEB_DR,'',$file_list[$i]['path']); ?>" target="_blank"><?php echo $file_list[$i]['filename']; ?></a></td>
	...
	if(eregi("\.($allow_edit)",$file_list[$i]['filename'])){
	?>
